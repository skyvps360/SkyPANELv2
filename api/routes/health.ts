...
import { getRateLimitHealthCheck, validateRateLimitConfiguration } from '../services/rateLimitConfigValidator.js';
...
router.get('/detailed', (req: Request, res: Response) => {
  try {
    ...
    const httpStatus = rateLimitHealth.status === 'error' ? 503 : 
                      rateLimitHealth.status === 'warning' ? 200 : 200;
  ...
  try {
    ...
    res.status(health.status === 'error' ? 503 : 200).json({
      ...
      configuration: {
        ...
        limits: {
          anonymous: `${config.rateLimiting.anonymousMaxRequests} requests per ${Math.round(config.rateLimiting.anonymousWindowMs / 60000)} minutes`,
          authenticated: `${config.rateLimiting.authenticatedMaxRequests} requests per ${Math.round(config.rateLimiting.authenticatedWindowMs / 60000)} minutes`,
          admin: `${config.rateLimiting.adminMaxRequests} requests per ${Math.round(config.rateLimiting.adminWindowMs / 60000)} minutes`
        },
...
router.get('/metrics', (req: Request, res: Response) => {
  try {
    ...
    if (windowMinutes < 1 || windowMinutes > 1440) { // Max 24 hours
    ...
    res.status(200).json({
      ...
      metrics: {
        ...
        rates: {
          anonymousRequestsPerMinute: Math.round((metrics.anonymousRequests / windowMinutes) * 100) / 100,
          authenticatedRequestsPerMinute: Math.round((metrics.authenticatedRequests / windowMinutes) * 100) / 100,
          adminRequestsPerMinute: Math.round((metrics.adminRequests / windowMinutes) * 100) / 100,
          violationsPerMinute: Math.round(((metrics.anonymousViolations + metrics.authenticatedViolations + metrics.adminViolations) / windowMinutes) * 100) / 100
        }
      }
    });
