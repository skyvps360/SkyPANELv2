...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { presetId: presetIdRaw, customPreset } = req.body as { presetId: string; customPreset?: unknown };
      ...
      const mergedCustomPreset = mergeCustomPreset(customPreset, currentConfig.customPreset);
...
router.get('/networking/rdns', authenticateToken, requireAdmin, async (_req: Request, res: Response) => {
  try {
    const result = await query(
    ...
    if (isMissingTableError(err)) {
      return res.json({ config: { rdns_base_domain: 'ip.rev.skyvps360.xyz' }, warning: 'networking_config table not found. Apply migrations.' });
    }
    console.error('Admin networking rDNS get error:', err);
    res.status(500).json({ error: err.message || 'Failed to fetch rDNS configuration' });
  }
});
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const baseDomainRaw: string = String(req.body.rdns_base_domain || '').trim();
      const baseDomain = baseDomainRaw.replace(/^\.+|\.+$/g, '');

      // Upsert: update latest row or insert a new one if none exists
      try {
        const latest = await query('SELECT id FROM networking_config ORDER BY updated_at DESC LIMIT 1');
        ...
        if (isMissingTableError(err)) {
          return res.status(400).json({ error: 'networking_config table not found. Apply migrations before updating.' });
        }
        throw err;
      }
    } catch (err: any) {
      console.error('Admin networking rDNS upsert error:', err);
      res.status(500).json({ error: err.message || 'Failed to save rDNS configuration' });
    }
  }
);
